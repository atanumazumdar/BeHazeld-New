...
interface FinanceActionResult<T> {
  success: boolean;
  data?: T;
  message?: string;
}
...
async function importFinanceCsv(
  ...
  return {
    success: true,
    data: (await response.json()) as FinanceImportResponse,
  };
}

export async function importAccountCodesAction(
  formData: FormData,
): Promise<FinanceActionResult<FinanceImportResponse>> {
  return importFinanceCsv('/api/v1/finance/accounts/import', formData, 'Failed to import account codes.');
}
