...
import type { PurchaseBillResponse, VendorResponse } from '@/types/purchase';

const API_URL = process.env.NEXT_PUBLIC_API_URL ?? 'http://localhost:8000';
const ACCESS_MAX_AGE = 30 * 60;
const REFRESH_MAX_AGE = 7 * 24 * 60 * 60;

interface PurchaseActionResult<T> {
  success: boolean;
  data?: T;
  message?: string;
}
...
async function getPurchaseData<T>(
  path: string,
  fallback: string,
): Promise<PurchaseActionResult<T>> {
  let response: Response;
  try {
...
export async function listPurchaseBillsAction(): Promise<PurchaseActionResult<PurchaseBillResponse[]>> {
  return getPurchaseData('/api/v1/purchases/bills?limit=100', 'Failed to load purchase bills.');
}

export async function listVendorsAction(): Promise<PurchaseActionResult<VendorResponse[]>> {
  return getPurchaseData('/api/v1/purchases/vendors', 'Failed to load vendors.');
}
